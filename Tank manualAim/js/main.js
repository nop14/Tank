...
function hosin_collision_left(x,y,t){
    ...
    for(var i=0;i<55;i++){
        ...
        temp=context.getImageData(x+(95-i)*Math.cos(t)+6*Math.sin(t),y+(95-i)*Math.sin(t)-6*Math.cos(t),1,1);
        sum += te();
    }
    /*for(var i=0;i<10;i++){
...
function update(){
    ...
    for(let k of ['w','s','d','a','k','j']){
        if(keystate[k]){
            ...
            else if(k=='k' && hosin_right ==0){
                theta+=Math.PI/90;
                theta=theta%(2*Math.PI);
            }
            else if(k=='j' && hosin_left==0){
                theta-=Math.PI/90;
                theta=theta%(2*Math.PI);
            }
        }
